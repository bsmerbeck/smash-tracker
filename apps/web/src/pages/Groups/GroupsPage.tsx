...
export function GroupsPage() {
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-6">
      ...
      {groups.isLoading && <div className="text-muted-foreground">{t('groups.loading')}</div>}
      ...
      {!groups.isLoading && myGroups.length === 0 && (
        <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed p-16 text-center">
          ...
          <h2 className="text-lg font-semibold">{t('groups.empty.title')}</h2>
          <p className="max-w-sm text-sm text-muted-foreground">{t('groups.empty.body')}</p>
        </div>
      )}
      ...
      {selectedGroupId && (
        <div className="flex flex-col gap-4 rounded-lg border p-4">
          {leaderboard.isLoading && (
            <div className="text-muted-foreground">{t('groups.leaderboardLoading')}</div>
          )}

          {leaderboard.isError && (
            <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
              {describeGroupsError(leaderboard.error, t('groups.leaderboardError'))}
            </div>
          )}
