...
describe('TREE_DESCRIPTORS lock', () => {
  it('the real registry exact-set-locks against TENANT_DELETION_TREES (22 members, 7 copy + 15 assert-empty)', () => {
    expect(TREE_DESCRIPTOR_LOCK.ok).toBe(true);
    expect(TREE_DESCRIPTORS).toHaveLength(22);
    expect(TREE_DESCRIPTORS.filter(isCopyDescriptor)).toHaveLength(7);
...
describe('runMigration', () => {
  it('a clean source migrates every copy tree and reports ok=true', async () => {
    ...
    expect(manifest.copies).toHaveLength(7);
    expect(manifest.trees).toHaveLength(7);
